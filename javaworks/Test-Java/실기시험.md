...
| 항목 | 내용 |
|---|---|
| 배점 | 문제1 5점 + 문제2 15점 + 문제3~4 10점 × 2 + 문제5~6 5점 × 2 = 50점 (필기 50점과 합산 시 100점) |
| 형식 | 새 프로젝트를 하나 생성하여, 아래 6개 문제를 순서대로 진행하는 미니 프로젝트 |
| 진행 방식 | 문제5는 문제4에서 이어집니다. DB 조회(문제4) → 정렬(문제5) 순으로 같은 사원 목록을 계속 사용하세요. 문제6은 독립적으로 진행합니다. |
...
## 문제 5. (5점) 람다식으로 급여순 정렬하기

문제 4에서 조회한 `List<Employee>`를 **급여(salary) 내림차순**으로 정렬하는 `EmployeeSorter`를 아래 클래스 다이어그램대로 작성하고, 정렬 결과를 출력하시오. (`salary`가 `null`인 사원은 없다고 가정)

```mermaid
classDiagram
    class EmployeeSorter {
        +sortBySalaryDesc(List~Employee~ empList) void
    }
```

- 정렬은 **Comparator 람다식**을 사용할 것

---

## 문제 6. (5점) 페이지 네비게이션 출력
